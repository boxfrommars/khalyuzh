...
- История рациона и веса переключается между полным списком и графиком до 40
  последних записей. График рациона показывает исторические границы нормы из
  snapshot каждой записи. Основная линия обоих графиков показывает скользящее
  среднее за 7 календарных дней, а вторичная линия и точки — точные значения;
  API-контракты при этом не меняются.
