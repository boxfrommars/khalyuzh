...
`app/config.php` задаёт путь к SQLite, часовой пояс `Asia/Yerevan`, постоянный
профиль питомца для отчёта, включая диагноз и окрас, параметры рациона, названия
кормов, калорийность и диапазон нормы. Изменение параметров рациона влияет
только на новые записи кормления; профиль питомца не копируется в постоянную базу.
